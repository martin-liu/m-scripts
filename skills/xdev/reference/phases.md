...
If a cold reset lands after a new sprint is appended but before its contract is approved, `Latest marker:` (`[APPROVED: PRODUCTION]`) and `Current sprint:` (integer N) will disagree — this trips the consistency check (write a `Delegation failure: Latest marker and Current sprint inconsistent` note under `## Escalations` and stop — only the user can repair this), which is the intended safety net.
